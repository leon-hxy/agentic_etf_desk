#!/usr/bin/env python3
"""Generate Stage 5 WP4 portfolio drift check artifacts."""
...
DRIFT_SCRIPT = ROOT / "scripts" / "portfolio" / "check_portfolio_drift.py"
...
WORK_PACKAGE = "Stage 5 WP4 drift checks"
WORK_PACKAGE_ID = "stage5_wp4_drift_checks"
NEXT_MAJOR_STAGE = "Stage 5"
NEXT_WORK_PACKAGE = "Stage 5 WP5 rebalance research ticket"
...
def _load_drift_checker() -> Any:
    spec = importlib.util.spec_from_file_location("check_portfolio_drift", DRIFT_SCRIPT)
...
def build_report(drift_payload: dict[str, Any]) -> dict[str, Any]:
    validation_checks = {
        "allowlist_source_recorded": drift_payload["universe_allowlist"] == "configs/universe/etf_universe.yaml",
        "benchmark_comparison_preserved": drift_payload["benchmark_comparison_preserved"] is True,
        "drift_artifact_written": DRIFT_JSON.exists(),
        "manual_trading_disclaimer_present": drift_payload["final_trading_manual"] is True,
        "no_trade_ticket_generated": drift_payload["trade_ticket_generated"] is False,
        "source_weights_exist": WEIGHTS_JSON.exists(),
        "universe_allowlist_enforced": drift_payload["universe_allowlist_enforced"] is True,
    }
    return {
        "asset_scope": "ETF-only",
        "automatic_trading": False,
        "benchmark_comparison_preserved": True,
        "benchmark_symbol": drift_payload["benchmark_symbol"],
        BROKER_ACCESS_SURFACE_FIELD: False,
        "drift_artifact": "data/portfolio/portfolio_drift_latest.json",
        "drift_status": drift_payload["status"],
        "drift_threshold": drift_payload["drift_threshold"],
        "final_trading_manual": True,
        "major_stage": NEXT_MAJOR_STAGE,
        "manual_trading_note": MANUAL_NOTE_EN,
        "max_absolute_drift": drift_payload["max_absolute_drift"],
        "max_drift_symbol": drift_payload["max_drift_symbol"],
        ...
        "risk_agent_review": {
            ...
            "scope": "drift checks are non-actionable portfolio monitoring and generate no trade ticket",
            "trade_tickets_actionable_without_review": False,
        },
        "source": drift_payload["source"],
        "status": "completed_internal_review",
        "target_strategy_id": drift_payload["target_strategy_id"],
...
def render_report_markdown(report: dict[str, Any], drift_payload: dict[str, Any]) -> str:
    rows = [
        f"| {row['symbol']} | {row['current_weight']:.2%} | {row['target_weight']:.2%} | {row['drift']:.2%} | {row['direction']} | {str(row['breaches_threshold']).lower()} |"
        for row in drift_payload["drift_rows"]
    ]
    return "\n".join(
        [
            ...
            *rows,
            ...
            "## Metrics",
            "",
            f"- Target strategy: {report['target_strategy_id']}",
            f"- Benchmark symbol: {report['benchmark_symbol']}",
            "- benchmark comparison: preserved.",
            f"- Drift status: {report['drift_status']}",
            f"- Drift threshold: {report['drift_threshold']:.2%}",
            f"- Max drift symbol: {report['max_drift_symbol']}",
            f"- Max absolute drift: {report['max_absolute_drift']:.2%}",
            "",
            "## Artifacts",
            "",
...
def build_internal_review() -> dict[str, Any]:
    changed_files = [
        "data/portfolio/portfolio_drift_latest.json",
        ...
        "reports/internal_reviews/program/stage5_wp4_drift_checks.md",
        ...
        "scripts/reports/generate_stage5_wp4_drift_checks.py",
        ...
        "tests/safety/test_stage5_wp4_drift_checks.py",
    ]
    return {
...
def render_internal_review_markdown(review: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# Stage 5 WP4 Drift Checks Internal Review",
            "",
            "## Metadata",
            "",
    ...
    state.update(
        {
            ...
            "last_internal_review": "reports/internal_reviews/program/stage5_wp4_drift_checks.json",
            "last_report": "reports/program_runner/stage5_wp4_drift_checks_report.json",
...
def append_heartbeat(now: str) -> None:
    ...
    for line in existing.splitlines():
        if line.startswith("## ") and " Stage 5 WP4" in line:
            skipping = True
            continue
...
def main() -> int:
    ...
    drift_payload = checker.check_portfolio_drift(WEIGHTS_JSON, output_json=DRIFT_JSON)
    report = build_report(drift_payload)
    internal_review = build_internal_review()

    _write_json(REPORT_JSON, report)
    REPORT_MD.write_text(render_report_markdown(report, drift_payload), encoding="utf-8")
    ...
    print(
        json.dumps(
            {
                "next_work_package": NEXT_WORK_PACKAGE,
                "report": "reports/program_runner/stage5_wp4_drift_checks_report.md",
                "status": "pass",
            },
            indent=2,
            sort_keys=True,
