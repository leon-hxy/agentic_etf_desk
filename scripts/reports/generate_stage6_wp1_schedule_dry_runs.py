#!/usr/bin/env python3
"""Generate Stage 6 WP1 repo-only schedule dry-run artifacts."""
...
WORK_PACKAGE = "Stage 6 WP1 schedule dry-runs"
WORK_PACKAGE_ID = "stage6_wp1_schedule_dry_runs"
NEXT_MAJOR_STAGE = "Stage 6"
NEXT_WORK_PACKAGE = "Stage 6 WP2 error recovery"
...
def build_schedule(now: str) -> dict[str, Any]:
    schedule_events = [
        {
            "asset_scope": "ETF-only",
            "cadence": "weekday_before_market_open",
            "event_id": "daily_market_brief",
            "execution_mode": "dry_run",
            "expected_artifact": "reports/stage2b_market_brief.md",
            "live_send_attempted": False,
            "real_runtime_modified": False,
            "repo_only_validation_command": "python3 scripts/reports/generate_market_brief.py",
            "service_restart_required": False,
        },
        {
            "asset_scope": "ETF-only",
            "cadence": "weekly_after_market_close",
            "event_id": "weekly_report",
            "execution_mode": "dry_run",
            "expected_artifact": "reports/stage2b_weekly_report.md",
            "live_send_attempted": False,
            "real_runtime_modified": False,
            "repo_only_validation_command": "python3 scripts/reports/generate_weekly_report.py",
            "service_restart_required": False,
        },
        {
            "asset_scope": "ETF-only",
            "cadence": "monthly_after_month_end",
            "event_id": "monthly_rebalance_research_ticket",
            "execution_mode": "dry_run",
            "expected_artifact": "reports/portfolio/stage5_wp5_rebalance_research_ticket.md",
            "live_send_attempted": False,
            "real_runtime_modified": False,
            "repo_only_validation_command": "python3 scripts/reports/generate_stage5_wp5_rebalance_research_ticket.py",
            "service_restart_required": False,
        },
        {
            "asset_scope": "ETF-only",
            "cadence": "daily_after_data_refresh",
            "event_id": "universe_health_check",
            "execution_mode": "dry_run",
            "expected_artifact": "reports/program_runner/stage4_wp5_universe_health_check_command_output_report.md",
            "live_send_attempted": False,
            "real_runtime_modified": False,
            "repo_only_validation_command": "python3 scripts/data/validate_universe.py",
            "service_restart_required": False,
        },
        {
            "asset_scope": "ETF-only",
            "cadence": "monthly_research_validation",
            "event_id": "backtest_command",
            "execution_mode": "dry_run",
            "expected_artifact": "reports/program_runner/stage4_wp6_backtest_command_output_report.md",
            "live_send_attempted": False,
            "real_runtime_modified": False,
            "repo_only_validation_command": "python3 scripts/backtest/run_backtest.py --strategy static_6040",
            "service_restart_required": False,
        },
    ]
    return {
        ...
        "created_at_utc": now,
        "dry_run_mode": "repo_only_render_and_validate",
        ...
        "real_runtime_modified": False,
        "repo_only": True,
        "report_type": "schedule_dry_run_plan",
        "risk_agent_review": {
            "new_actionable_trade_suggestion": False,
            "result": "passed",
            "reviewer": "risk_agent",
            "scope": "schedule dry-run plan; no trade ticket or actionable trade suggestion generated",
            "trade_ticket_actionable_without_review": False,
        },
        "schedule_events": schedule_events,
        "scheduler_binding": "documentation_only_no_launchd_or_cron_change",
...
def render_schedule_markdown(schedule: dict[str, Any]) -> str:
    rows = [
        (
            f"| {event['event_id']} | {event['cadence']} | {event['execution_mode']} | "
            f"`{event['repo_only_validation_command']}` | {str(event['live_send_attempted']).lower()} |"
        )
    ...
    return "\n".join(
        [
            ...
            *rows,
            ...
            "## Next Safe Action",
            "",
            f"Proceed to `{schedule['next_work_package']}`.",
            "",
        ]
    )


def build_report(schedule: dict[str, Any]) -> dict[str, Any]:
    validation_checks = {
        "benchmark_comparison_required": schedule["benchmark_comparison_required"] is True,
        "dry_run_events_defined": len(schedule["schedule_events"]) == 5,
        "live_send_not_attempted": schedule["live_send_attempted"] is False,
        "manual_trading_disclaimer_present": schedule["final_trading_manual"] is True,
        "real_runtime_not_modified": schedule["real_runtime_modified"] is False,
        "schedule_artifact_written": SCHEDULE_JSON.exists(),
        "services_not_restarted": schedule["services_restarted"] is False,
        "trade_ticket_not_generated": schedule["trade_ticket_generated"] is False,
    }
    return {
        "asset_scope": "ETF-only",
        "automatic_trading": False,
        ...
        "risk_agent_review": schedule["risk_agent_review"],
        "schedule_artifacts": {
            "json": "reports/operations/stage6_wp1_schedule_dry_runs.json",
            "markdown": "reports/operations/stage6_wp1_schedule_dry_runs.md",
        },
        "services_restarted": False,
...
def render_report_markdown(report: dict[str, Any], schedule: dict[str, Any]) -> str:
    return "\n".join(
        [
            ...
            "## Schedule Result",
            "",
            f"- Dry-run event count: {len(schedule['schedule_events'])}.",
...
def build_internal_review() -> dict[str, Any]:
    changed_files = [
        ...
        "reports/internal_reviews/program/stage6_wp1_schedule_dry_runs.md",
        "reports/operations/stage6_wp1_schedule_dry_runs.json",
        "reports/operations/stage6_wp1_schedule_dry_runs.md",
        "reports/program_runner/stage6_wp1_schedule_dry_runs_report.json",
        "reports/program_runner/stage6_wp1_schedule_dry_runs_report.md",
        "reports/review_requests/latest.json",
        "scripts/reports/generate_stage6_wp1_schedule_dry_runs.py",
        "tests/safety/test_program_runner_governance.py",
        "tests/safety/test_stage6_wp1_schedule_dry_runs.py",
    ]
    return {
...
def render_internal_review_markdown(review: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# Stage 6 WP1 Schedule Dry-Runs Internal Review",
            "",
            "## Metadata",
            "",
            "- major_stage: Stage 6",
    ...
    state.update(
        {
            ...
            "last_internal_review": "reports/internal_reviews/program/stage6_wp1_schedule_dry_runs.json",
            "last_report": "reports/program_runner/stage6_wp1_schedule_dry_runs_report.json",
...
def append_heartbeat(now: str) -> None:
    ...
    for line in existing.splitlines():
        if line.startswith("## ") and " Stage 6 WP1" in line:
            skipping = True
            continue
...
def main() -> int:
    ...
    _write_json(SCHEDULE_JSON, schedule)
    SCHEDULE_MD.write_text(render_schedule_markdown(schedule), encoding="utf-8")

    report = build_report(schedule)
    internal_review = build_internal_review()
    _write_json(REPORT_JSON, report)
    REPORT_MD.write_text(render_report_markdown(report, schedule), encoding="utf-8")
    ...
    print(
        json.dumps(
            {
                "next_work_package": NEXT_WORK_PACKAGE,
                "report": "reports/program_runner/stage6_wp1_schedule_dry_runs_report.md",
                "schedule": "reports/operations/stage6_wp1_schedule_dry_runs.md",
                "status": "pass",
            },
            indent=2,
            sort_keys=True,
