...
class Stage31Wp2RealDataQualityTest(unittest.TestCase):
    ...
    def test_wp2_internal_review_and_latest_artifacts_are_internal_only(self) -> None:
        result = self.run_cmd(["scripts/reports/generate_stage3_1_wp2_artifacts.py"])
        self.assertEqual(result.returncode, 0, msg=result.stdout + result.stderr)
        ...
        self.assertEqual(handoff["next_work_package"], "WP3 formal backtest and evidence package")
        ...
        self.assertEqual(runner["current_work_package"], "WP3 formal backtest and evidence package")
        self.assertEqual(runner["status"], "wp2_completed_internal_review_ready_for_wp3")
