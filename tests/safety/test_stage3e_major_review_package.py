...
class Stage3EMajorReviewPackageTest(unittest.TestCase):
    ...
    def test_stage3e_updates_governance_state_after_internal_review(self) -> None:
        ...
        self.assertEqual(review_request["stage"], "Stage 3.1 Real ETF Historical Data MVP scope consolidated")
        self.assertEqual(review_request["review_target"], "Stage 3.1 scope consolidation")
        self.assertFalse(review_request["request_chatgpt_review_for_finalization_fixes"])
        self.assertEqual(review_request["review_level"], "scope_consolidation")
